...
public class Launcher {
    ...
    public long launch(final ResultWriter aWriter, final String aClasspath,
    ...
    private long launchJProfilerPhase(final String aClasspath,
        ...
        // Step 3: Attach via JDI and listen for child VM termination, do not dispose vm so events are delivered
        ...
        // Wait for natural termination; JDI event thread will handle cleanup on VMDeath.
    ...
    private Process launchJVMSuspended(final String javaPath,
        ...
        System.out.println("Launched JVM in suspended mode with PID: " +
                process.pid() + " at directory:" +
    ...
    // Programmatically attach debugger (JDWP) and resume the suspended JVM using JDI
    ...
    private void runProfilerCleanupAPI(long jvmPid) {
        try {
            ...
            if (snapshotFile.exists()) {
                System.out.println("Snapshot file created successfully: " + snapshotPath + " ( " + snapshotFile.length() + " bytes)");
            } else {
    ...
    public static void executeJpcontrollerCommand(final String command,
                                                  final String parameter, long jvmPid) throws Exception {
        try {
    ...
    private void stopProfiling(long jvmPid) throws Exception {
        ...
        ProcessBuilder pb = new ProcessBuilder(Constants.JPCONTROLLER_PATH,
                "-n", "-f", Constants.PROJECT_ROOT + "/Output/JProfiler/command.txt");
    ...
    /**
     *
     The following code was added to support JMX-based control of JProfiler for starting/stopping recordings.
     So far, it is not needed now; but just kept just in case if we ever require to move to JMX control in the future.
     */


    public long launchWithJMXProfiling(final ResultWriter aWriter, final String aClasspath,
                                       final String aFQN, final String... programArgs) throws Exception {
...
//            controller.stopAllRecordings();
            ...
            if (jvmProcess.waitFor(WAIT_TIMEOUT_MS, TimeUnit.MILLISECONDS)) {
                ...
                System.err.println("Timed out waiting for profiled JVM to exit after " + WAIT_TIMEOUT_MS + "ms. Forcing termination...");
...
//            controller.saveSnapshot(snapshotPath);
    ...
    private void attachShutdownListener(final VirtualMachine vm, final String mainClass, JProfilerRemoteControllerClient controller) {
        Thread eventThread = new Thread(() -> {
            try {
                EventRequestManager erm = vm.eventRequestManager();
