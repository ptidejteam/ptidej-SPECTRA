...
public class CSVMerger {
	public static void main(String[] args) throws Exception {
		...
		Map<String, MethodData> allObjectsEnergyData = mergeAndFilterAllObjectsEnergyData(allObjectsCsvPath,
				energyCsvPath);
	...
	public static Map<String, MethodData> mergeAndFilterEnergyData(Map<String, MethodData> methodDataMap,
			String energyCsvPath, boolean isXML) throws IOException {
		Map<String, MethodData> mergedData = new HashMap<>();

		try (Reader reader = new FileReader(energyCsvPath);
				CSVParser csvParser = new CSVParser(reader,
						CSVFormat.DEFAULT.withHeader("method_name", "energy(Joules)").withIgnoreSurroundingSpaces()
								.withIgnoreEmptyLines().withTrim().withSkipHeaderRecord(true))) {
	...
	public static Map<String, MethodData> mergeAndFilterAllObjectsEnergyData(String allObjectsCsvPath,
			String energyCsvPath) throws IOException {
		Map<String, MethodData> mergedData = new HashMap<>();
	...
	private static void writeToExcel(Map<String, MethodData> methodDataMap, String filePath) throws IOException {
		try (Workbook workbook = new XSSFWorkbook()) {
			...
			Row headerRow = sheet.createRow(0);
			String[] headers = { "Method Signature", "Invocations", "Execution Time", "Instance Count", "Size (bytes)",
					"Energy (J)" };
			createHeaderRow(headerRow, headers, workbook);
