...
class SuperOverloadTest {
	public static int add(int x, int y) {
		long end = System.currentTimeMillis() + 20000;
		while (System.currentTimeMillis() < end) {
			Math.log(x + y); // Dummy CPU load
		}
		System.out.println("Executing add(int, int) in Super_OverloadTest");
		return (int) (x + y);
	}
	...
	public double add(double x, double y) {
		...
		System.out
				.println("Executing add(double, double) in Super_OverloadTest");
		return x + y;
	}

}

public class OverloadTest extends SuperOverloadTest {
	public static int add(int x, int y) {
		long end = System.currentTimeMillis() + 20000;
		while (System.currentTimeMillis() < end) {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		final SuperOverloadTest test3 = new SuperOverloadTest();
